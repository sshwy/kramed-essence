...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    // escape
    if (cap = this.rules.escape.exec(src), /*console.log(`escape: src: ^${src}$ cap: ^${cap}$`) ,*/ cap) {
    ...
    // autolink
    if (cap = this.rules.autolink.exec(src), cap) {
      src = src.substring(cap[0].length);
      if (cap[2] === '@') {
        text = cap[1].charAt(6) === ':'
    ...
    // link
    if (cap = this.rules.link.exec(src), cap) {
      src = src.substring(cap[0].length);
      this.inLink = true;
    ...
    // label
    if ((cap = this.rules.label.exec(src))) {
      src = src.substring(cap[0].length);
      out += this.renderer.label(cap[1]);
      continue;
    }
    ...
    // strong
    if (cap = this.rules.strong.exec(src), cap) {
      src = src.substring(cap[0].length);
    ...
    // em
    if (cap = this.rules.em.exec(src), cap) {
      src = src.substring(cap[0].length);
    ...
    // br
    if (cap = this.rules.br.exec(src), cap) {
      src = src.substring(cap[0].length);
    ...
    // del (gfm)
    if (cap = this.rules.del.exec(src), cap) {
      src = src.substring(cap[0].length);
    ...
    if (cap = this.rules.text.exec(src), cap) {
      //console.log(`text: src: ^${src}$, cap: ^${cap[0]}$`);
      src = src.substring(cap[0].length);
