...
function kramed(src, opt, callback) {
  if (callback || typeof opt === 'function') { // 没有给 opt
    if (!callback) {
      callback = opt;
      opt = null;
    }
    ...
    for (let i = 0; i < tokens.length; i++) { // 把代码高亮做了
      (function(token) {
        if (token.type !== 'code') {
          return --pending || done(); // 如果 pending === 0 就 done()
        }
        return highlight(token.text, token.lang, function(err, code) { // 源代码, 语言, 回调函数
