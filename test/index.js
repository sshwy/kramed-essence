...
function runTests (eng, opts) {
  if (typeof eng !== 'function') {
    opts = eng;
    eng = null;
  }

  let originalOpts = merge({}, kramed.defaults, (opts && opts.kramed) || {});
  let engineOpts;
  let defaultEngine = function (src) {
    return kramed(src, engineOpts);
  };

  let engine = eng || defaultEngine
    , options = opts || {}
...
function parseArg (/*argv*/) {
  ...
  while (argv.length) {
    ...
    switch (arg) {
      ...
      default:
        if (arg.indexOf('--') === 0) {
          let opt = camelize(arg.replace(/^--(no-)?/, ''));
          if (!Object.prototype.hasOwnProperty.call(kramed.defaults, opt)) {
            continue;
          }
